...
class TMDBClient {
    ...
    enum Endpoints {
        ...
        var stringValue: String {
            ...
            case .getPopular:
                return Endpoints.base + "/movie/popular" + Endpoints.apiKeyParam + "&language=en-US"
            }
        }
    ...
    class func getPopular( completion: @escaping ([Movie], Error?) -> Void) -> URLSessionDataTask {
        let task = taskForGETRequest(url: Endpoints.getPopular.url, responseType: MovieResults.self) { response, error in
            if let response = response {
                completion(response.results, nil)
            } else {
                completion([], error)
